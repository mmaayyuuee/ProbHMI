...
from utils.thops import rotmat_to_sixd, rotmat_to_sixd_torch, sixd_to_rotmat, sixd_to_rotmat_torch


## codes from https://github.com/wei-mao-2019/HisRepItself/blob/master/utils/ang2joint.py
...
def ang2joint(p3d0, pose, parent):
    """
...
def ang2joint_6d(p3d0, pose, parent):
    """
    :param p3d0:[batch_size, joint_num, 3]
    :param pose:[batch_size, joint_num, 6]
    :param parent:
    :return:
    """
    # batch_num = p3d0.shape[0]
    ...
    jnum = len(parent)
    J = p3d0
    R_cube_big = sixd_to_rotmat_torch(pose.contiguous().view(-1, 1, 6)).reshape(batch_num, -1, 3, 3)
    results = []
    results.append(
        with_zeros(torch.cat((R_cube_big[:, 0], torch.reshape(J[:, 0, :], (-1, 3, 1))), dim=2))
    )

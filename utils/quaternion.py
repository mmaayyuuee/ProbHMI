...
class Quaternion(object):
    ...
    def _from_axis_angle(cls, axis, angle):
        ...
        """
        norm = torch.norm(axis, dim=-1)[..., None] + 1e-9
        axis_n = axis / norm
        theta = angle.unsqueeze(-1) / 2.0
        r = torch.where(norm > 1e-9, torch.cos(theta), torch.ones_like(theta))
        i = torch.where(norm > 1e-9, axis_n*torch.sin(theta), torch.zeros_like(axis_n))

...
class TimePoolingHead(nn.Module):
    def __init__(self, hidden_size, patch_size, out_channels):
        super().__init__()
        self.linear = nn.Linear(hidden_size, patch_size * patch_size * out_channels, bias=True)
        return
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.cond_embedder = nn.Linear(48, hidden_size, bias=True)      # fixed input size here !!!
        # Will use fixed sin-cos embedding:
        self.pos_embed = nn.Parameter(torch.zeros(1, node_n, hidden_size), requires_grad=False)
    ...
    def initialize_weights(self):
        ...
        # pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], int(self.node_n ** 0.5))
        pos_embed = sincos_pos_embed(self.node_n, self.pos_embed.shape[-1])
    ...
    def forward(self, x, t, x_cond=None, label=None, **kwargs):
        """
        Forward pass of DiT.
        x: (B, C, N) tensor of spatial inputs (poses or latent representations of poses)
        ...
        if label is not None:
            label = self.y_embedder(label, self.training)
            c = c + label                        # (N, D)
    ...
    def get_layer_params_size(self, prefix=''):
        ...
        for name, params in self.named_parameters():
            named_size[prefix+'DiT.'+name] = params.size()
...
class DiT_V2(nn.Module):
    ...
    def __init__(
        ...
        self.y_embedder = LabelEmbedder(num_classes, hidden_size, 0.1)
        # Will use fixed sin-cos embedding:
        self.pos_embed = nn.Parameter(torch.zeros(1, length, hidden_size), requires_grad=False)

        self.blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])
        self.final_layer = FinalLayer(hidden_size, 1, self.out_channels)
        self.final_head = TimePoolingHead(length, 1, 1)
        self.initialize_weights()
    ...
    def initialize_weights(self):
        ...
        nn.init.constant_(self.final_head.linear.weight, 0)
        nn.init.constant_(self.final_head.linear.bias, 0)
        return

    def forward(self, x, t, x_cond=None, label=None, **kwargs):
        """
        Forward pass of DiT.
        x: (B, T, C) tensor of spatial inputs (poses or latent representations of poses)
        t: (B,) tensor of diffusion timesteps
        """        
        if x_cond is not None:
            x = torch.concat((x_cond, x), dim=1)
        ...
        for block in self.blocks:
            x = block(x, c)                      # (B, T, D)
        x = x.permute(0, 2, 1)
        x = self.final_head(x)                   # (B, 1, out_channels)
        x = x.permute(0, 2, 1)
...
class DiT_V3(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.final_layer = FinalLayer(hidden_size, 1, self.model_out_channels)
        self.initialize_weights()
        return
